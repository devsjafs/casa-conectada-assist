...
serve(async (req) => {
  ...
  try {
    ...
    if (error) {
      console.error('SmartThings OAuth error:', error);
      return new Response(null, {
        status: 302,
        headers: { Location: `${redirectUrl}?smartthings=error&message=${encodeURIComponent(error)}` },
      });
    }
    ...
    // Sync devices using the new token
    try {
